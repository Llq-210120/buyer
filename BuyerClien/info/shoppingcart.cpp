...
ShoppingCart::ShoppingCart(const QString &buyerId, const QString &bookId, const QString &price,
             const QString &number, const QString & time, const QString &condition, const QImage &image)
{
    m_buyerId =  buyerId ;
    m_bookId =  bookId;
    m_price = price;
...
ShoppingCart::ShoppingCart(const ShoppingCart &other){
    m_buyerId =  other.getBuyerId();
    m_bookId =  other.getBookId();
...
bool operator==(ShoppingCart s1,ShoppingCart s2){

    if(s2.getBookId() == s1.getBookId())
            return true;
}
